CREATE TABLE IF NOT EXISTS "product_db_harness_smoke" (
    "id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    "name" varchar(64) NOT NULL,
    "creator" varchar(64) DEFAULT '',
    "create_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updater" varchar(64) DEFAULT '',
    "update_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "deleted" bit NOT NULL DEFAULT FALSE,
    PRIMARY KEY ("id")
) COMMENT '持久化基座冒烟表';

CREATE TABLE IF NOT EXISTS "product_info" (
    "id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    "product_code" varchar(32) NOT NULL,
    "name" varchar(128) NOT NULL,
    "product_type" tinyint NOT NULL DEFAULT 1,
    "auth_type" tinyint NOT NULL DEFAULT 0,
    "status" tinyint NOT NULL DEFAULT 0,
    "version" varchar(16) NOT NULL DEFAULT 'v1',
    "description" varchar(512) DEFAULT '',
    "cache_enabled" bit NOT NULL DEFAULT FALSE,
    "async_support" bit NOT NULL DEFAULT FALSE,
    "need_auth_no" bit NOT NULL DEFAULT FALSE,
    "remark" varchar(512) DEFAULT '',
    "creator" varchar(64) DEFAULT '',
    "create_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updater" varchar(64) DEFAULT '',
    "update_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "deleted" bit NOT NULL DEFAULT FALSE,
    PRIMARY KEY ("id"),
    CONSTRAINT "uk_product_info_code" UNIQUE ("product_code")
) COMMENT '产品信息表';

CREATE TABLE IF NOT EXISTS "product_function" (
    "id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    "func_code" varchar(32) NOT NULL,
    "name" varchar(128) NOT NULL,
    "product_id" bigint NOT NULL,
    "sort" int NOT NULL DEFAULT 0,
    "required" bit NOT NULL DEFAULT FALSE,
    "charge" bit NOT NULL DEFAULT FALSE,
    "status" tinyint NOT NULL DEFAULT 0,
    "remark" varchar(512) DEFAULT '',
    "creator" varchar(64) DEFAULT '',
    "create_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updater" varchar(64) DEFAULT '',
    "update_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "deleted" bit NOT NULL DEFAULT FALSE,
    PRIMARY KEY ("id"),
    CONSTRAINT "uk_product_function_code" UNIQUE ("func_code")
) COMMENT '产品功能表';

CREATE TABLE IF NOT EXISTS "product_func_interface" (
    "id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    "product_function_id" bigint NOT NULL,
    "ds_interface_id" bigint NOT NULL,
    "ds_interface_code" varchar(32) DEFAULT '',
    "priority" int NOT NULL DEFAULT 0,
    "is_default" bit NOT NULL DEFAULT FALSE,
    "status" tinyint NOT NULL DEFAULT 0,
    "creator" varchar(64) DEFAULT '',
    "create_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updater" varchar(64) DEFAULT '',
    "update_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "deleted" bit NOT NULL DEFAULT FALSE,
    PRIMARY KEY ("id")
) COMMENT '产品功能-数据源接口绑定表';

CREATE TABLE IF NOT EXISTS "product_param" (
    "id" bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    "product_id" bigint NOT NULL,
    "param_direction" tinyint NOT NULL,
    "field_name" varchar(128) NOT NULL,
    "data_type" tinyint NOT NULL DEFAULT 1,
    "required" bit NOT NULL DEFAULT FALSE,
    "validation_rule" varchar(256) DEFAULT '',
    "desensitize_rule" varchar(128) DEFAULT '',
    "description" varchar(256) DEFAULT '',
    "sort" int NOT NULL DEFAULT 0,
    "creator" varchar(64) DEFAULT '',
    "create_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updater" varchar(64) DEFAULT '',
    "update_time" datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "deleted" bit NOT NULL DEFAULT FALSE,
    PRIMARY KEY ("id")
) COMMENT '产品参数定义表';
